...
class Larynx_DataModule(pl.LightningDataModule):
    ...
    def __init__(self, data_dir: str = "./", batch_size: int = 2, spatial_size: int = 128):
        super().__init__()
    ...
    def setup(self, stage: Optional[str] = None):
        if stage in (None, 'fit'):
            self.dataset_train = Larynx_Data(root=self.data_dir, mode="train", spatial_size=self.spatial_size)
            self.dataset_val = Larynx_Data(root=self.data_dir, mode="val", spatial_size=self.spatial_size)
